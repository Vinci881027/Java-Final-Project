...
public class PlayController implements Initializable {
	...
	@FXML
	private Rectangle _bloodRec1P, _flashRec1P, _bloodRec2P, _flashRec2P;
	private int _bloodNum1P = 100, _flashNum1P = 100, _bloodNum2P = 100, _flashNum2P = 100;
	@FXML
	private Label _time;
	@FXML
	private Text _VS;
	@FXML
	private Circle _clock;
	@FXML
	private ImageView _scene, _role1, _role2, _role3, _role4, _role5;
	@FXML
	private ImageView _weapon1, _weapon2, _weapon3, _weapon4, _weapon5;
	...
	public void initialize(URL arg0, ResourceBundle arg1) { // initialize
		...
		} else if (MainController.gameMode == 2) {
			_VS.setVisible(false);
			Timeline fpsClock = new Timeline(new KeyFrame(Duration.millis(1000), (e) -> {
				if (Integer.parseInt(_time.getText()) == 0) {
					try {
						if (_bloodNum1P > _bloodNum2P)
							winner = 1;
						else if (_bloodNum1P < _bloodNum2P)
							winner = 2;
						else
							winner = 3;
						gameOver();
				...
				if (Integer.parseInt(_time.getText()) >= 0)
					_time.setText(Integer.toString(Integer.parseInt(_time.getText()) - 1));
				if (Integer.parseInt(_time.getText()) <= 10 && Integer.parseInt(_time.getText()) >= 7) {
					_time.setTextFill(Color.web("ff0000"));
					Media media = new Media(getClass().getResource("../resource/clockTicking.mp3").toString());
					clockTicking = new MediaPlayer(media);
					clockTicking.play();
				}
			}));
			fpsClock.setCycleCount(Timeline.INDEFINITE);
			fpsClock.play();
		}
		...
		KeyFrame KWeaponMove = new KeyFrame(Duration.millis(1), (e) -> { // let weapon move
			...
			for (var b : tBulletsA) {
				...
				if (b.getLayoutX() + b.getFitWidth() / 2 > _role2P.getLayoutX() + _role2P.getFitWidth() / 2 - 1
						&& b.getLayoutX() + b.getFitWidth() / 2 < _role2P.getLayoutX() + _role2P.getFitWidth() / 2 + 1
						&& b.getLayoutY() + b.getFitHeight() > _role2P.getLayoutY()
						&& b.getLayoutY() < _role1P.getLayoutY() + _role2P.getFitHeight()) {
					_bloodNum2P = _bloodNum2P - 2;
			...
			for (var b : tBulletsD) {
				b.setLayoutX(b.getLayoutX() + 1);
				if (b.getLayoutX() > _role2P.getLayoutX() - 1 && b.getLayoutX() < _role2P.getLayoutX() + 1
						&& b.getLayoutY() + b.getFitHeight() > _role2P.getLayoutY()
...
//						jumpCount1P = 50;
			...
			if (_role2P.getLayoutY() + _role2P.getFitHeight() < 848
					...
					&& !(_role2P.getLayoutY() + _role2P.getFitHeight() > 520
							...
							&& ((_role2P.getLayoutX() + _role2P.getFitWidth() > 22 && _role2P.getLayoutX() < 420)
